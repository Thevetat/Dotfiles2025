...
#*
## Name: start-rolling
## Desc: Start ***REDACTED*** development environment with Rails server and Vite in tmux session
## Inputs: None
## Usage: start-rolling
start-rolling() {
    # Change to the Rails directory
    builtin cd ~/Git/***REDACTED***/***REDACTED***_rails || {
        echo "❌ Cannot find ***REDACTED*** Rails directory"
        return 1
    }
        ...
        if tmux has-session -t main 2>/dev/null; then
            if [ "$current_session" = "main" ]; then
                ...
                if [ "$pane_count" -lt 2 ]; then
                    ...
                    tmux split-window -h -c ~/Git/***REDACTED***/***REDACTED***_rails 'bin/vite dev'
            ...
            tmux new-session -d -s main -c ~/Git/***REDACTED***/***REDACTED***_rails 'bundle exec rails s'
            
            # Split vertically and start Vite
            tmux split-window -h -t main -c ~/Git/***REDACTED***/***REDACTED***_rails 'bin/vite dev'

...
- [ ] Set each machine's local dotfiles repo to fast-forward-only pulls.
...
- [ ] Keep agents from using `dotfiles add -A` or `dotfiles add .`.
...
- [ ] Keep portable aliases/functions in shared tracked files.
- [ ] Split `.aliases` into shared, OS-specific, host-specific, and local/private pieces.
- [ ] Split NVM/rbenv/conda startup by OS and interaction mode.
- [ ] Remove duplicate Codex wrapper definitions and keep one source of truth.
...
- [ ] Move sensitive project paths, client targets, and private repo shortcuts out of public tracked files.
